...
class PiperDownloaderDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        inst_label = wx.StaticText(
            self.panel_instaladas, label=_("Voces que ya tienes en tu equipo:")
        )
        instaladas_sizer.Add(inst_label, 0, wx.ALL, 10)

        self.list_instaladas = wx.ListBox(self.panel_instaladas, style=wx.LB_SINGLE)
        instaladas_sizer.Add(
            self.list_instaladas, 1, wx.EXPAND | wx.LEFT | wx.RIGHT, 10
        )
        ...
        lang_label = wx.StaticText(
            self.panel_descargar,
            label=_("Selecciona los idiomas para filtrar las voces:"),
        )
        descargar_sizer.Add(lang_label, 0, wx.ALL, 10)

        self.lang_list = wx.ListCtrl(
            self.panel_descargar, style=wx.LC_REPORT | wx.LC_NO_HEADER
        )
        ...
        voice_label = wx.StaticText(
            self.panel_descargar, label=_("Selecciona la voz que deseas descargar:")
        )
