...
class DiscordTokenDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        super().__init__(
            parent, wx.ID_ANY, title=_("Se necesita el token del bot de Discord")
        )
        ...
        intro = wx.StaticText(
            ...
            wx.ID_ANY,
            _(
                "Para leer un canal de Discord, VeTube necesita el token de tu bot. Pégalo aquí; quedará guardado y no se te volverá a pedir."
            ),
        )
        intro.Wrap(420)
        ...
        self.boton_guia = wx.Button(
            self, wx.ID_ANY, _("¿No sabes cómo crear el token? Abrir la &guía")
        )
    ...
    def on_aceptar(self, event):
        if not self.get_token():
            wx.MessageBox(
                _("Debes pegar el token del bot para continuar."),
                _("Error"),
                wx.ICON_ERROR,
            )
            self.token_ctrl.SetFocus()
