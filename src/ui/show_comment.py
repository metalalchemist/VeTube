...
class ShowCommentDialog(wx.Dialog):
    def __init__(self, parent, text):
        super().__init__(
            parent, title=_("Mensaje"), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
        )
        main_sizer = wx.BoxSizer(wx.VERTICAL)
        self.text_message = wx.TextCtrl(
            self,
            wx.ID_ANY,
            text,
            style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_PROCESS_ENTER,
        )
        ...
        self.choice_idiomas = wx.Choice(
            self, wx.ID_ANY, choices=[self.tw.LANGUAGES[k] for k in self.tw.LANGUAGES]
        )
        current_lang_code = languageHandler.curLang[:2]
        if current_lang_code in self.tw.LANGUAGES:
            self.choice_idiomas.SetStringSelection(self.tw.LANGUAGES[current_lang_code])
    ...
    def cambiarTraducir(self, event):
        self.traducir.SetLabel(
            _("&Traducir mensaje")
            if self.choice_idiomas.GetString(self.choice_idiomas.GetSelection())
            != self.tw.LANGUAGES[languageHandler.curLang[:2]]
            else _("&Traducir al idioma del programa")
        )
        ...
        for k in self.tw.LANGUAGES:
            if self.tw.LANGUAGES[k] == self.choice_idiomas.GetStringSelection():
                self.text_message.SetValue(
                    self.tw.translate(self.text_message.GetValue(), target=k)
                )
                break
        self.label_mensaje_texto.SetLabel(
            _("mensaje en ")
            + self.choice_idiomas.GetString(self.choice_idiomas.GetSelection())
        )
        self.text_message.SetFocus()
