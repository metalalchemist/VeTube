...
class NuevaCombinacionDialog:
    def __init__(
        self, parent, combinaciones, indice=None, texto=None, description=None
    ):
        # Remove logic related to indice and mensaje_teclas for description
        # The description is now passed directly
        self.dlg_editar_combinacion = wx.Dialog(
            parent,
            wx.ID_ANY,
            _("Editando la combinación de teclas para %s") % description,
        )
        sizer = wx.BoxSizer(wx.VERTICAL)
        groupbox = wx.StaticBox(
            self.dlg_editar_combinacion,
            wx.ID_ANY,
            _("Selecciona las teclas que quieres usar"),
        )
        sizer_groupbox = wx.StaticBoxSizer(groupbox, wx.VERTICAL)
        sizer_check = wx.BoxSizer(wx.HORIZONTAL)
        self.check_ctrl = wx.CheckBox(
            self.dlg_editar_combinacion, wx.ID_ANY, _("&Control")
        )
        if "control" in texto:
            self.check_ctrl.SetValue(True)
        self.check_alt = wx.CheckBox(self.dlg_editar_combinacion, wx.ID_ANY, _("&Alt"))
        if "alt" in texto:
            self.check_alt.SetValue(True)
        self.check_shift = wx.CheckBox(
            self.dlg_editar_combinacion, wx.ID_ANY, _("&Shift")
        )
        if "shift" in texto:
            self.check_shift.SetValue(True)
        self.check_win = wx.CheckBox(
            self.dlg_editar_combinacion, wx.ID_ANY, _("&Windows")
        )
        if "win" in texto:
            self.check_win.SetValue(True)
        ...
        label_tecla = wx.StaticText(
            self.dlg_editar_combinacion,
            wx.ID_ANY,
            _("Selecciona una tecla para la combinación"),
        )
        self.combo_tecla = wx.ComboBox(
            self.dlg_editar_combinacion,
            wx.ID_ANY,
            choices=self.teclas,
        ...
        self.editar = wx.Button(
            self.dlg_editar_combinacion,
            wx.ID_OK,
            _("&Aplicar nueva combinación de teclado"),
        )
        close = wx.Button(self.dlg_editar_combinacion, wx.ID_CLOSE, _("&Cerrar"))
