...
class PanelSonidos(wx.Panel):
    def __init__(self, parent):
        ...
        if (
            not self.lista_temas.SetStringSelection(config["directorio"])
            and self.lista_temas.GetCount() > 0
        ):
        ...
        label_dispositivo = wx.StaticText(
            self, wx.ID_ANY, _("Seleccionar dispositivo de audio")
        )
