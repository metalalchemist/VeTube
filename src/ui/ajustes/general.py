...
from globals.resources import codes, langs
...
class PanelGeneral(wx.Panel):
    def __init__(self, parent):
        ...
        label_language = wx.StaticText(
            self, wx.ID_ANY, _("Idioma de VeTube (Requiere reiniciar)")
        )
        ...
        self.check_donaciones = wx.CheckBox(
            self, wx.ID_ANY, _("Activar diálogo de donaciones al inicio de la app.")
        )
        self.check_donaciones.SetValue(config["donations"])
        boxSizer_1.Add(self.check_donaciones, 0, wx.ALL, 5)
        self.check_salir = wx.CheckBox(
            self, wx.ID_ANY, _("preguntar si desea salir de la app al cerrar.")
        )
        self.check_salir.SetValue(config["salir"])
        boxSizer_1.Add(self.check_salir, 0, wx.ALL, 5)
        self.check_actualizaciones = wx.CheckBox(
            self, wx.ID_ANY, _("Comprobar si hay actualizaciones al iniciar la app")
        )
        ...
        self.choice_canal = wx.Choice(
            self,
            wx.ID_ANY,
            choices=[
                _("Estable (solo versiones oficiales)"),
                _("Beta (incluye versiones de prueba)"),
            ],
        )
        ...
        self.check_backup = wx.CheckBox(
            self, wx.ID_ANY, _("Crear backup de seguridad antes de actualizar")
        )
        self.check_backup.SetValue(config.get("create_backup_before_update", True))
        boxSizer_1.Add(self.check_backup, 0, wx.ALL, 5)
        self.check_traduccion = wx.CheckBox(
            self,
            wx.ID_ANY,
            _("intentar traducir las novedades cuando salga una actualización."),
        )
        self.check_traduccion.SetValue(config["traducir"])
        boxSizer_1.Add(self.check_traduccion, 0, wx.ALL, 5)
        self.check_interface = wx.CheckBox(
            self, wx.ID_ANY, _("Desactivar la interfaz invisible")
        )
