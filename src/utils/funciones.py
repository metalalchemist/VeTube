...
def retornarCategorias():
    ...
    lista = [
        ...
        if i < len(config["categorias"]) and config["categorias"][i]
    ]
...
def convertirLista(lista, val1, val2):
    if len(lista) <= 0:
        ...
        newlista = []
        for datos in lista:
            newlista.append(datos[val1] + ": " + datos[val2])
        return newlista
...
def extractUser(url):
    ...
    if start_index == -1:
        ...
        if start_index == -1:
            return None  # ningún usuario reconocible en el enlace
        start_index += 3
