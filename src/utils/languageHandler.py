        ...
        if LCID != 0:
            ...
            if "_" not in language:
                res = ctypes.windll.kernel32.GetLocaleInfoW(
                    LCID, LOCALE_SLANGDISPLAYNAME, buf, 1024
                )
            else:
                res = 0
            if res == 0:
                res = ctypes.windll.kernel32.GetLocaleInfoW(
                    LCID, LOCALE_SLANGUAGE, buf, 1024
                )
            desc = buf.value
    return desc
...
def getAvailableLanguages():
    ...
    l = [
        x
        for x in os.listdir(LOCALES_DIR)
        if (LOCALES_DIR / x / "LC_MESSAGES" / "VeTube.mo").is_file()
    ]
    ...
    try:
        if lang == "system":
            ...
            trans = gettext.translation(
                "VeTube", localedir=str(LOCALES_DIR), languages=[localeName]
            )
            curLang = localeName
        else:
            trans = gettext.translation(
                "VeTube", localedir=str(LOCALES_DIR), languages=[lang]
            )
            curLang = lang
            localeChanged = False
            if system == "Windows":
