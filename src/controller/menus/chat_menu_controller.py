...
class ChatMenuController:
    ...
    def _bind_menu_events(self):
        ...
        self.parent.Bind(
            wx.EVT_MENU,
            lambda evt: (
                self.chat_controller.main_controller.mostrar_editor_combinaciones()
            ),
            self.menu.editor_combinaciones,
        )
        self.parent.Bind(wx.EVT_MENU, self.addFavoritos, self.menu.favoritos)
        self.parent.Bind(
            wx.EVT_MENU, self.mostrar_estadisticas, self.menu.ver_estadisticas
        )
        self.parent.Bind(wx.EVT_MENU, self.copiarEnlace, self.menu.copiar_enlace)
        self.parent.Bind(
            wx.EVT_MENU, self.reproducirVideo, self.menu.reproducir_navegador
        )
        self.parent.Bind(
            wx.EVT_MENU,
            lambda evt: self.chat_controller.buscar_mensajes(),
            self.menu.buscar,
        )
    ...
    def addFavoritos(self, event):
        ...
        main_frame = (
            self.chat_controller.frame
        )  # ventana principal; GetParent() daría el notebook del ChatDialog
        list_favorite = main_frame.list_favorite
        url = self.chat_controller.servicio.url

        if not url:
            wx.MessageBox(
                _("No hay una URL para agregar a favoritos."),
                _("Aviso"),
        ...
        if self.plataforma == "TikTok":
            titulo = funciones.extractUser(url) or url
        elif (
            self.plataforma == "Twich"
            and self.chat_controller.servicio.chat.status != "past"
        ):
        ...
        if any(fav.get("url") == url for fav in favorite):
            wx.MessageBox(
                _("Ya se encuentra en favoritos"),
                _("Aviso"),
                wx.OK | wx.ICON_INFORMATION,
            )
            return

        if list_favorite.GetCount() > 0 and list_favorite.GetStrings()[0] == _(
        ...
        wx.MessageBox(
            _("Se ha agregado a favoritos"), _("Aviso"), wx.OK | wx.ICON_INFORMATION
        )
    ...
    def copiarEnlace(self, event):
        ...
        if url:
            ...
            copy(url)
            noti = wx.adv.NotificationMessage(
                _("Enlace copiado al portapapeles"),
                _("El enlace del chat ha sido copiado al portapapeles."),
            )
            noti.Show(timeout=5)
        else:
            wx.adv.NotificationMessage(
                _("No se pudo copiar el enlace"),
                _("No se encontró un enlace válido para copiar."),
            ).Show(timeout=5)
    ...
    def reproducirVideo(self, event):
        ...
        if url:
            ...
            wx.adv.NotificationMessage(
                ...
                _("No se encontró un enlace válido para abrir."),
            ).Show(timeout=5)

    def mostrar_estadisticas(self, event):
        controller = EstadisticasController(
            self.parent, self.estadisticas_manager, self.plataforma
        )
        controller.show()
