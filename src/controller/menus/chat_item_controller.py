import wx
import wx.adv
from pyperclip import copy

from globals.data_store import mensajes_destacados
from globals.paths import MENSAJES_DESTACADOS_FILE
from setup import reader
from ui.list_urls import ListUrlsDialog
from ui.show_comment import ShowCommentDialog
from utils.funciones import escribirJsonLista, extractUser
from utils.translator import TranslatorWrapper


class ChatItemController:
    def __init__(self, menu, list_box, chat_controller):
        self.menu = menu
        self.list_box = list_box
        self.chat_controller = chat_controller
        self.plataforma = chat_controller.plataforma
        self.label_dialog = chat_controller.ui.label_dialog

        # Bind events
        self.menu.menu.Bind(wx.EVT_MENU, self.on_copy, id=self.menu.copiar.GetId())
        self.menu.menu.Bind(wx.EVT_MENU, self.on_show, id=self.menu.mostrar.GetId())
        self.menu.menu.Bind(
            wx.EVT_MENU, self.on_translate, id=self.menu.traducir.GetId()
        )
        self.menu.menu.Bind(
            wx.EVT_MENU, self.on_list_urls, id=self.menu.listado_urls.GetId()
        )
        self.menu.menu.Bind(wx.EVT_MENU, self.on_archive, id=self.menu.archivar.GetId())

    def on_copy(self, event):
        selected_text = self.list_box.GetString(self.list_box.GetSelection())
        copy(selected_text)
        noti = wx.adv.NotificationMessage(
            _("Mensaje copiado al portapapeles"),
            _("El mensaje seleccionado ha sido copiado al portapapeles."),
        )
        noti.Show(timeout=10)

    def on_show(self, event):
        selected_text = self.list_box.GetString(self.list_box.GetSelection())
        dlg = ShowCommentDialog(self.menu.parent, selected_text)
        dlg.ShowModal()
        dlg.Destroy()

    def on_translate(self, event):
        selected_text = self.list_box.GetString(self.list_box.GetSelection())
        translator = TranslatorWrapper()
        from utils.languageHandler import curLang

        copy(translator.translate(selected_text, target=curLang[:2]))
        noti = wx.adv.NotificationMessage(
            _("Mensaje traducido"),
            _(
                "el mensaje se ha traducido al idioma del programa y se  a  copiado en el portapapeles."
            ),
            parent=self.menu.parent,
        )
        noti.Show(timeout=10)

    def on_list_urls(self, event):
        ListUrlsDialog(self.list_box, self.menu.parent)

    def on_archive(self, event):
        main_frame = (
            self.chat_controller.frame
        )  # ventana principal; GetParent() daría el notebook del ChatDialog
        list_mensajes = main_frame.list_mensajes
        if list_mensajes.GetCount() > 0 and list_mensajes.GetStrings()[0] == _(
            "Tus mensajes archivados aparecerán aquí"
        ):
            list_mensajes.Delete(0)

        mensaje = self.list_box.GetString(self.list_box.GetSelection())
        if not mensaje:
            return  # Evitar archivar mensajes vacíos

        ya_archivado = any(mensaje == d.get("mensaje", "") for d in mensajes_destacados)
        if not ya_archivado:
            # Determinar el título primero
            if self.plataforma == "TikTok":
                titulo = (
                    extractUser(self.chat_controller.servicio.url)
                    or self.chat_controller.servicio.url
                )
            else:
                titulo = self.label_dialog.GetLabelText()

            # Añadir a la lista de la UI y al almacenamiento de datos
            list_mensajes.Append(f"{mensaje}: {titulo}")
            mensajes_destacados.append({"mensaje": mensaje, "titulo": titulo})

            escribirJsonLista(MENSAJES_DESTACADOS_FILE, mensajes_destacados)
            reader.leer_aviso(_("El mensaje ha sido archivado correctamente."))
        else:
            reader.leer_aviso(_("Este mensaje ya está en la lista de archivados."))
