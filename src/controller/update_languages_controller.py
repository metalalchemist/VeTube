...
class UpdateLanguagesController:
    ...
    def _prepare_display_list(self):
        ...
        for lang_code, version in actualizaciones.items():
            languages_to_display.append(
                _("%s (Actualizar a v%s)") % (lang_code, version)
            )
    ...
    def start_update(self, event=None):
        ...
        if not selected_languages_text:
            wx.MessageBox(
                _("Por favor, selecciona al menos un idioma para actualizar."),
                _("Advertencia"),
        ...
        async def do_all_updates():
            ...
            for lang_info in languages_to_process:
                tasks.append(
                    self._download_and_install_language_async(
                        lang_info["code"], lang_info["version"]
                    )
                )
        ...
        try:
            ...
            if result["success"]:
                wx.CallAfter(
                    ...
                    _("Idioma %s actualizado con éxito.") % lang_code,
                )
            else:
                error_msg = result.get("data", "Error desconocido")
                wx.CallAfter(
                    self.view.set_status, _("Error en %s: %s") % (lang_code, error_msg)
                )
                wx.CallAfter(
                    wx.MessageBox,
                    _("Error al actualizar %s: %s") % (lang_code, error_msg),
                    _("Error"),
                    wx.ICON_ERROR,
                )
            ...
            wx.CallAfter(
                self.view.set_status, _("Error inesperado en %s: %s") % (lang_code, exc)
            )
            wx.CallAfter(
                wx.MessageBox,
                _("Error inesperado en %s:\n%s\n\nVer consola para más detalles.")
                % (lang_code, exc),
                _("Error"),
                wx.OK | wx.ICON_ERROR,
            )
    ...
    def _finalize_update(self):
        ...
        def _logic():
            ...
            self.view.ok_button.Bind(
                wx.EVT_BUTTON, lambda evt: self.view.EndModal(wx.ID_OK)
            )
            wx.EndBusyCursor()
    ...
    def close(self, event=None):
        if self.update_completed:
            if (
                response(
                    _(
                        "Es necesario reiniciar el programa para aplicar los nuevos idiomas. ¿Desea reiniciarlo ahora?"
                    ),
                    _("¡Atención!"),
                )
                == wx.ID_YES
