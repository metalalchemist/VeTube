    ...
    def recibir(self):
        try:
            ...
            self.chat = ChatDownloader().get_chat(
                self.url,
                message_groups=["messages", "bits", "subscriptions", "upgrades"],
            )
            threading.Thread(
            ...
            for message in self.chat:
                ...
                if (
                    ...
                    sub_message = ""
                    if (
                        message["message_type"] == "resubscription"
                        and data_store.config["eventos"][2]
                    ):
                        sub_message = _(
                            "{author_name} ha renovado su suscripción en el nivel {subscription_plan_name}. ¡Lleva suscrito por {cumulative_months} meses!"
                        ).format(
                            author_name=author_name,
                            subscription_plan_name=message.get(
                                "subscription_plan_name", ""
                            ),
                            cumulative_months=message.get("cumulative_months", ""),
                        )
                        ...
                        sub_message = _(
                            "{author_name} se ha suscrito en el nivel {subscription_plan_name} por {cumulative_months} meses!"
                        ).format(
                            author_name=author_name,
                            subscription_plan_name=message.get(
                                "subscription_plan_name", ""
                        ...
                        message["message_type"] == "mystery_subscription_gift"
                        and data_store.config["eventos"][2]
                    ):
                        sub_message = _(
                            "{author_name} regaló una suscripción de nivel {subscription_type} a la comunidad, ¡ha regalado un total de {sender_count} suscripciones!"
                        ).format(
                            author_name=author_name,
                            subscription_type=message.get("subscription_type", ""),
                            sender_count=message.get("sender_count", ""),
                        )
                    elif (
                        message["message_type"] == "subscription_gift"
                        and data_store.config["eventos"][2]
                    ):
                        sub_message = _(
                            "{author_name} ha regalado una suscripción a {gift_recipient_display_name} en el nivel {subscription_plan_name} por {number_of_months_gifted} meses!"
                        ).format(
                            author_name=author_name,
                            gift_recipient_display_name=message.get(
                                "gift_recipient_display_name", ""
                            ),
                            subscription_plan_name=message.get(
                                "subscription_plan_name", ""
                            ),
                            number_of_months_gifted=message.get(
                                "number_of_months_gifted", ""
                            ),
                        )
                    wx.CallAfter(
                    ...
                    if data_store.config["reader"] and data_store.config["unread"][2]:
                        wx.CallAfter(reader.leer_mensaje, sub_message)
                    continue
                ...
                if (
                    ...
                    and data_store.config["eventos"][3]
                    ...
                    clean_msg = re.sub(
                        r"cheer\d+", "", msg, flags=re.IGNORECASE
                    ).strip()
                    # Si el mensaje queda vacío después de quitar los Cheers, no poner los dos puntos
                    full_donacion = f"{dinero}, {author_name}" + (
                        f": {clean_msg}" if clean_msg else ""
                    )
                ...
                try:
                    if (
                        message["author"].get("is_moderator")
                        ...
                        message["author"].get("is_subscriber")
                    ...
                    for badge in message["author"]["badges"]:
                        ...
                        if "Subscriber" in title:
                            ...
                            if (
                                ...
                                and data_store.config["categorias"][4]
                                and hasattr(
                                    self.chat_controller.ui, "list_box_moderadores"
                                )
                            ):
                                if (
                                    data_store.config["sonidos"]
                                    ...
                                    wx.CallAfter(player.play, rutasonidos[4])
                                wx.CallAfter(
                                    self.chat_controller.agregar_mensaje_moderador,
                                    full_message,
                                )
                                if (
                                    data_store.config["reader"]
                            ...
                            if (
                                ...
                                and hasattr(
                                    self.chat_controller.ui, "list_box_verificados"
                                )
                            ):
                                wx.CallAfter(
