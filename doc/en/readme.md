...
If TikTok's usual chat service is down, there's a browser-based alternative; see the [extension guide](tiktok_navegador.md).
## Features

- Automatic mode: Read incoming  chat messages in real time using your preferred sapi5 voice
- Invisible interface: Manage chats from any window using simple key commands. You’ll need to have a screen reader running.
- Supported screen readers:
  ...
  - PC Talker
  - ZDSR
-  customize and configure to fit your needs.
  - Enable or disable program sounds.
  - Turn automatic mode on or off.
  - Configure the list of messages in the invisible interface.
  - Set voice preferences.
- Change the reading mode for chats easily: decide if you want to hear all chats, members or donations.
- Save your livestreams on a favorites section; access the chat easily without needing to find the link again.
- Translate a  chat to another language.
...
- The ability to show information about the person who is chatting in the invisible interface, including:
  - The user's channel name
  - Additional relevant details

## Contribute to translation
If you want to collaborate by translating VeTube into your language, you will need to install the internationalization tools.
...
2.  **Extract texts to update the template (.pot):**
    If new strings have been added to the code, update the template file:
    ```bash
    pybabel extract -F babel.cfg -o vetube.pot .
    ```

3.  **Start a new translation:**
    If you are translating to a new language (e.g., `it` for Italian):
    ```bash
    pybabel init -i vetube.pot -d locales -l it -D vetube
    ```

4.  **Update existing translations:**
    If the language already exists and you have updated the `.pot`, synchronize the `.po` files:
    ```bash
    pybabel update -i vetube.pot -d locales -D vetube
    ```

5.  **Compile translations:**
    For the program to recognize the changes, compile the `.po` files to `.mo`:
    ```bash
...
Who took part on the  graphic design for the project and fixed minor bugs.

[AnimalMetal](https://www.twitter.com/AnimalMetal),
for improving the english translation of the program and the readme.

I’m confident that, thanks to all of you, this application will continue to improve, with each of your ideas and contributions being added.

For ideas, bug reports, or any suggestions you can drop me a message at 
...
[Download the 64-bit portable version of the program here](https://github.com/metalalchemist/VeTube/releases/latest/download/vetube-v3.96.zip)
[Download the 64-bit installer version of the program here](https://github.com/metalalchemist/VeTube/releases/latest/download/vetube-v3.96.msi)
[Download the 32-bit portable version of the program here](https://github.com/metalalchemist/VeTube/releases/download/v3.7/VeTube-x86.zip)
