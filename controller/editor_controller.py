...
from globals.mensajes import comandos_a_descripcion
...
class EditorController:
    ...
    def on_restore_defaults(self, event):
        if (
            response(
                _(
                    "Está apunto de restaurar las combinaciones a sus valores por defecto, ¿desea proceder? Esta acción no se puede desacer."
                ),
                _("Atención:"),
            )
            == wx.ID_YES
    ...
    def on_editar(self, event):
        ...
        dlg = NuevaCombinacionDialog(
            self.ui.dlg_teclado,
            self.ui.combinaciones,
            texto=selected_key,
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if shift:
                self.nueva_combinacion = "shift+" + self.nueva_combinacion
            if alt:
                self.nueva_combinacion = "alt+" + self.nueva_combinacion
            if ctrl:
                self.nueva_combinacion = "control+" + self.nueva_combinacion
            if win:
                self.nueva_combinacion = "win+" + self.nueva_combinacion
            ...
            for busc in range(self.ui.combinaciones.GetItemCount()):
                ...
                if (
                    ...
                    == self.ui.combinaciones.GetItem(busc, 1).GetText()
                ):
                    wx.MessageBox(
                        _("Esta combinación ya está siendo usada en la función %s")
                        % comandos_a_descripcion.get(
                            config["atajos_chat"].get(
                                self.ui.combinaciones.GetItem(busc, 1).GetText(), ""
                            ),
