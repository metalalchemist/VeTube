...
class ChatDialogController:
            ...
            for key, command_str in config[section].items():
                try:
                    ...
                    if obj_name == "chat":
                        ...
                        handler = lambda m=method_name: (
                            self._resolve_method(self.active_chat_session, m)()
                        ...
                        handler = lambda m=method_name: (
                            self._resolve_method(
                                self.globally_controlled_media_player, m
                            )()
    ...
    def on_close(self, event):
        ...
        if not self.is_programmatic_close:
            if (
                wx.MessageBox(
                    _("¿Desea cerrar la ventana de lectura de los chats en vivo?"),
                    _("¡Atención!"),
    ...
    def on_media_player_state_change(self, media_controller, state):
        if state == "playing":
            if (
                self.globally_controlled_media_player
                and self.globally_controlled_media_player != media_controller
            ):
