...
from globals.data_store import (
    config,
    favorite,
    favs,
    mensajes_destacados,
...
class MainController:
            ...
            if not lf.GetStrings()[0] == _("Tus mensajes archivados aparecerán aquí"):
                if len(mensajes_destacados) > 0:
                    ...
                    funciones.escribirJsonLista(
                        MENSAJES_DESTACADOS_FILE, mensajes_destacados
                    )
                    lf.SetFocus()
                else:
                    wx.MessageBox(
    ...
    def abrir_chat_dialog(self, event=None, url=""):
        ...
        if url:
            ...
            if (
                ...
                def handle_tiktok_result(result):
                    ...
                    try:
                        if isinstance(result, Exception):
                            wx.MessageBox(
                                _("Error al simplificar URL de TikTok: {}").format(
                                    result
                                ),
                                _("Error"),
                                wx.ICON_ERROR,
                            ...
                            wx.MessageBox(
                                _("No se pudo obtener la URL real de TikTok."),
                                _("Error"),
                                wx.ICON_ERROR,
                            )
                    finally:
                ...
                network.execute(
                    canonical_scraper.get_simplified_tiktok_live_url(url),
                    callback=handle_tiktok_result,
                )
            else:
    ...
    def _continuar_abriendo_chat(self, url):
        ...
        if not autodetectar:
            ...
            if plataforma_ids == 1:
                ...
                plataforma_ids == 6
            ):  # Discord: no hay URL construible, se necesita el enlace del canal
                wx.MessageBox(
                    _(
                        "Para Discord pega el enlace del canal de texto: haz clic derecho sobre el canal y elige «Copiar enlace»."
                    ),
                    _("Error"),
                    wx.ICON_ERROR,
                )
        ...
        if "yout" in url:
            ...
            if not extraer_id_canal(url):
                wx.MessageBox(
                    _(
                        "El enlace de Discord no es válido. Copia el enlace del canal de texto (clic derecho sobre el canal, «Copiar enlace») y pégalo en VeTube."
                    ),
                    _("Error"),
                    wx.ICON_ERROR,
                )
        ...
        try:
            if plataforma_ids == 1:
                ...
                plataforma_ids == 6
            ):  # Discord (el token ya quedó validado en la detección del enlace)
                servicio = ServicioDiscord(
                    self, url, self.frame, plataforma, chat_controller
                )
    ...
    def mostrar_editor_combinaciones(self, event=None):
        ...
        if self.chat_dialog and self.chat_dialog.view.IsShown():
            parent = (
                self.chat_dialog.view
            )  # Si el chat dialog está visible, él es el padre
    ...
    def _pedir_token_discord(self, url):
        ...
        def al_validar(valido):
            ...
            if valido is False:
                wx.MessageBox(
                    _(
                        "El token no es válido. Comprueba que lo copiaste completo desde el portal de desarrolladores de Discord."
                    ),
                    _("Error"),
                    wx.ICON_ERROR,
                )
                ...
                wx.MessageBox(
                    _(
                        "No se pudo comprobar el token por un fallo de red. Revisa tu conexión a internet e inténtalo de nuevo."
                    ),
                    _("Error"),
                    wx.ICON_ERROR,
                )
    ...
    def close(self):
        ...
        if self.chat_dialog:
            try:
                ...
                for url, (controller, page_index) in list(
                    self.chat_dialog.chat_sessions.items()
                ):
