...
from utils import app_utilitys, fajustes
...
class MainMenuController:
    ...
    def _bind_menu_events(self):
        ...
        self.frame.Bind(
            ...
            self.menu.itemPageMain,
        )
        self.frame.Bind(
            wx.EVT_MENU,
            lambda evt: updater.do_update(is_manual=True),
            self.menu.actualizador,
        )
        ...
        self.frame.Bind(
            wx.EVT_MENU,
            self.main_controller.mostrar_editor_combinaciones,
            self.menu.opcion_0,
        )
    ...
    def mostrar_ajustes(self, event):
        ...
        try:
            if resultado == wx.ID_OK:
                self.config_dialog = (
                    dlg  # Guarda la referencia para usarla en guardar()
                )
                self.guardar()
            else:
    ...
    def restaurar(self, event):
        if (
            response(
                _(
                    "Estás apunto de reiniciar la configuración a sus valores predeterminados, ¿Deseas proceder?"
                ),
                _("Atención:"),
            )
            == wx.ID_YES
    ...
    def on_update_languages(self, event):
        if self.checking_languages:
            ...
            wx.MessageBox(
                _("Ya hay una comprobación de idiomas en curso."),
                _("Actualización de idiomas"),
                wx.ICON_INFORMATION,
            )
            return
        self.checking_languages = True
        gestor = GestorRepositorios(
            self.frame, github_repo="metalalchemist/vetube", local_dir="."
        )
        ...
        def handle_result(result):
            ...
            if isinstance(result, Exception):
                wx.MessageBox(
                    _("Error al conectar con el servidor de idiomas: {}").format(
                        result
                    ),
                    _("Error de red"),
                    wx.ICON_ERROR,
        ...
        network.execute(
            gestor.comprobar_nuevos_y_actualizaciones(), callback=handle_result
        )

    def _handle_language_check_result(self, gestor, result):
        if not result["success"] and not result.get("error", True):
            wx.MessageBox(
                result["data"], _("Actualización de idiomas"), wx.ICON_INFORMATION
            )
        elif result["success"]:
            ...
            if nuevos or actualizaciones:
                ...
                update_controller = UpdateLanguagesController(
                    self.frame, gestor, result["data"]
                )
                update_controller.show()
                update_controller.close()  # This will be called after ShowModal returns
            else:
                wx.MessageBox(
                    _("No hay actualizaciones ni nuevos idiomas disponibles."),
                    _("Actualización de idiomas"),
                    wx.ICON_INFORMATION,
                )
    ...
    def comprobar_idiomas_al_inicio(self):
        ...
        def handle_result(result):
            ...
            if idioma_activo not in data.get(
                "nuevos", {}
            ) and idioma_activo not in data.get("actualizaciones", {}):
                return
        ...
        network.execute(
            asyncio.wait_for(gestor.comprobar_nuevos_y_actualizaciones(), 30),
    ...
    def _preguntar_actualizar_idioma(self, gestor, data):
        ...
        if (
            response(
                _(
                    "Hay una actualización disponible para el idioma del programa. ¿Quieres instalarla ahora?"
                ),
                _("Actualización de idiomas"),
            )
            == wx.ID_YES
    ...
    def guardar(self):
        ...
        data_store.config["categorias"] = [
            cf.categoriza.IsItemChecked(i) for i in range(cf.categoriza.GetItemCount())
        ]
        data_store.config["listasonidos"] = [
            cf.soniditos.IsItemChecked(i) for i in range(cf.soniditos.GetItemCount())
        ]
        data_store.config["eventos"] = [
            cf.eventos.IsItemChecked(i) for i in range(cf.eventos.GetItemCount())
        ]
        data_store.config["unread"] = [
            cf.unread.IsItemChecked(i) for i in range(cf.unread.GetItemCount())
        ]
        ...
        data_store.dst = (
            codigos_traduccion[seleccion_traduccion]
            if seleccion_traduccion != wx.NOT_FOUND
            else ""
        )
        rest = False
        ...
        data_store.config["update_channel"] = canal_seleccionado
        ...
        if rest:
            if (
                response(
                    _(
                        "Es necesario reiniciar el programa para aplicar el nuevo idioma. ¿desea reiniciarlo ahora?"
                    ),
                    _("¡Atención!"),
                )
                == wx.ID_YES
        ...
        if motor in ("piper", "kokoro"):
            ...
            reader._lector.set_rate(
                app_utilitys.porcentaje_a_escala(data_store.config["speed"])
            )
