...
class MyFrame(wx.Frame):
	...
	def estadisticas(self, event):
		...
		for i in range(len(self.mensajes)): self.mayor_menor.InsertStringItem(i, self.usuarios[i])
		for i in range(len(self.mensajes)): self.mayor_menor.SetStringItem(i, 1, str(self.mensajes[i]))
		...
		button_estadisticas_descargar = wx.Button(self.dlg_estadisticas, wx.ID_ANY, _("&Descargar las estadísticas en un archivo de texto"))
	...
	def descargarEstadisticas(self, event):
		...
		dlg_file.SetFilename("Estadísticas de mi canal")
		if dlg_file.ShowModal() == wx.ID_OK:
			nombre_archivo = dlg_file.GetFilename()
			directorio = dlg_file.GetDirectory()
			archivo = open(path.join(directorio, nombre_archivo), "w")
			# poner un texto que diga el título del canal
			archivo.write(_("Estadísticas del canal: %s %s") % (self.chat.title, self.chat.status)+ "\n")
			archivo.write(_("Total de usuarios: %s\nTotal de mensajes: %s") % (len(self.usuarios), sum(self.mensajes))+ "\n")
			archivo.write(_("Usuarios y mensajes:")+ "\n")
			arreglo = []
			for i in range(len(self.mensajes)): arreglo.append(self.usuarios[i] + " " + str(self.mensajes[i]))
			arreglo.sort()
			for i in range(len(self.mensajes)): archivo.write(arreglo[i] + "\n")
			archivo.close()
		dlg_file.Destroy()
